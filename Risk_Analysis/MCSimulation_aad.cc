...
template <class T>
MCSimulation<T>::MCSimulation(T fx_initial, T fx_vol, T r_dom, T r_foreign, T maturity, int num_sim) 
...
template <class T>
T MCSimulation<T>::MC_Simulation(T K, int num_step){
    ...
    for(int i = 0; i < num_sim; ++i){
        ...
        // Since Control-Flow can not be used in AAD
        //payoff = (fx_rate - K > static_cast<T>(0.0)) ? (fx_rate - K) : static_cast<T>(0.0);
        
        payoff = max(fx_rate - K, static_cast<T>(0.0)); 
...
template class MCSimulation<Number>;